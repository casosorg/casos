...
function getMenuParentKey(uri) {
  ...
  if (uri.includes("/helm-releases")) {return "/workloads";}
  ...
  function getMenuItems() {
    const allItems = [
      ...
      Setting.getItem(<Link to="/pods">{i18next.t("general:Workloads")}</Link>, "/workloads", <AppstoreOutlined />, [
        ...
        Setting.getItem(<Link to="/helm-releases">{i18next.t("general:Helm Releases")}</Link>, "/helm-releases"),
  ...
  function renderRouter() {
    return (
      <Switch>
        ...
        <Route exact path="/helm-releases" render={(props) => <HelmReleaseListPage {...props} />} />
