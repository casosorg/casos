...
// Worth a log button only when there is a step of our own to have gone wrong.
const hasEnvironmentSteps = (box) => Boolean(box?.repo || box?.hasSetup);

const hasSsh = (box) => Boolean(box?.sshHost && box?.sshPort);

// A workspace with a run still in flight is frozen on purpose; one that is
// merely stopped is not, and the two should not offer the same buttons.
...
/**
 * What the clone and the setup script printed on the workspace's latest start —
 * the first place to look when either badge turns red.
 */
...
function DevboxPage() {
  ...
  function choosePreset(preset) {
    setForm((prev) => ({
      ...prev,
      preset: preset.key,
      // A custom image keeps whatever was typed; a preset brings its own.
