...
const (
	...
	// Pinned: the copy script depends on file names inside it.
	...
	// Delete it to run the setup script again.
	devboxSetupMarker = devboxHomeMount + "/.casos/setup-done"
)

// The Alpine sshd needs musl, so it and its helpers run through the bundled musl loader.
...
// sshd needs a passwd entry for uid 1000, which most images lack.
...
// Never overwrites an existing checkout, and exits 0 on failure so the editor still starts.
...
// SetEnv must be one line (sshd keeps only the first); the shell stays PID 1 to reap sessions.
...
// Credentials in the URL are refused: they would be stored in the Deployment.
...
func applyDevboxEnvironment(env devboxEnvironment) func(*appsv1.Deployment) error {
	return func(depl *appsv1.Deployment) error {
		...
		if !ok {
			// A stateless box still needs a home shared by every step.
		...
		// The image's conda belongs to root; keep environments in home.
		...
		if terminated := status.State.Terminated; terminated != nil {
			...
			case devboxCloneInit:
				for _, line := range strings.Split(terminated.Message, "\n") {
					...
					switch key {
					case "branch":
						// Detached HEAD (a tag): the commit is enough.
						if value != "HEAD" {
							state.branch = value
						}
					case "commit":
